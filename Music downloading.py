...
async def _(mucis: Matcher, city: Message = Arg(), city_name: str = ArgPlainText("city")):
    global mucis_ids
    mucis_name_or_ids = await mucis_name_get(nameis=city_name)
    mucis_name = mucis_name_or_ids[0]
    mucis_ids = mucis_name_or_ids[1]
...
@hello.got(key='mucis', prompt='请输入序号下载')
async def a(xuhao: str = EventPlainText()):
    ids = mucis_ids['result']['songs'][int(xuhao)]['id']
    url = await mucis_url(ids=ids)
    await hello.send(f'{url}\n 复制此URL进入浏览器即可下载音乐' )
      ...
      , q = l.BufferedBlockAlgorithm = t.extend({
        ...
        mN0x: function(a) {
            var c = this.i6c
              ...
              , k = this.blockSize
              , b = j / (4 * k)
              , b = a ? u.ceil(b) : u.max((b | 0) - this.Rj3x, 0);
            a = b * k;
            j = u.min(4 * a, j);
            if (a) {
                for (var q = 0; q < a; q += k)
    ...
    var p = CryptoJS
      ...
      , v = d.Cipher = t.extend({
        ...
        createDecryptor: function(e, a) {
            return this.create(this.bvb9S, e, a)
        },
        init: function(e, a, b) {
...
(function() {
    ...
    var H = [0, 1, 2, 4, 8, 16, 32, 64, 128, 27, 54]
      , d = d.AES = p.extend({
        mx0x: function() {
            for (var a = this.J6D, c = a.words, d = a.sigBytes / 4, a = 4 * ((this.bil3x = d + 6) + 1), e = this.buZ9Q = [], j = 0; j < a; j++)
                if (j < d)
                    e[j] = c[j];
                else {
                    var k = e[j - 1];
        ...
        encryptBlock: function(a, b) {
            this.Fn5s(a, b, this.buZ9Q, t, r, w, v, l)
        },
        decryptBlock: function(a, c) {
            var d = a[c + 1];
            a[c + 1] = a[c + 3];
            a[c + 3] = d;
            this.Fn5s(a, c, this.buV9M, b, x, q, n, s);
            d = a[c + 1];
            a[c + 1] = a[c + 3];
            a[c + 3] = d
        },
        Fn5s: function(a, b, c, d, e, j, l, f) {
            for (var m = this.bil3x, g = a[b] ^ c[0], h = a[b + 1] ^ c[1], k = a[b + 2] ^ c[2], n = a[b + 3] ^ c[3], p = 4, r = 1; r < m; r++)
...
function biFromDecimal(a) {
    ...
    if (c == a.length)
        ...
        c += f; c < a.length; )
            d = biAdd(biMultiply(d, lr10), biFromNumber(
                Number(a.substr(c, dpl10)))),
            c += dpl10;
        d.isNeg = b
    }
    return d
...
function name() {
    ...
    var d =  {"hlpretag":"<span class=\\"s-fc7\\">","hlposttag":"</span>","s":"'''+str(nameis)+'''","type":"1","offset":"0","total":"true","limit":"30","csrf_token":""};
    var e ="010001";
    ...
    var bKB9s = asrsea(JSON.stringify(d),e,f,g);
    return bKB9s
}
console.log(name)'''
    cts = execjs.compile(js)
    exe = cts.call('name')
    url = 'https://music.163.com/weapi/cloudsearch/get/web?csrf_token='
    data = {
        'params': exe['encText'],
        'encSecKey': exe['encSecKey']
    }
    resp = httpx.post(url, headers=hand, data=data).json()
    # for i in range(10):
    #     # +':', jsons['result']['songs'][i]['id'])
    #     mucis_name = f'{i}:', resp['result']['songs'][i]['name']
...
{mucis_name_9}
        '''
    return mucis_name, resp

async def mucis_url(ids):
    jss = '''var CryptoJS = CryptoJS || function(u, p) {
    var d = {}
      , l = d.lib = {}
      , s = function() {}
      , t = l.Base = {
...
function start() {
    var d={"ids":"['''+str(ids)+''']","level":"standard","encodeType":"aac","csrf_token":""};
    var g ="0CoJUm6Qyw8W8jud";
    var e = "010001";
    var f="";
